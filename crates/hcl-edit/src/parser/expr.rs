...
pub(super) fn expr<'a>(input: &mut Input<'a>) -> PResult<'a, Expression> {
    let state = RefCell::new(ExprParseState::default());
    let _ = expr_inner(&state).parse_next(input)?;
...
fn expr_inner<'i, 's>(
    ...
    move |input: &mut Input<'i>| {
        ...
        loop {
            ...
            if let Ok(peek) = peek(take::<_, _, ParseError<_>>(2usize)).parse_next(input) {
                match peek {
                    ...
                    [b'.' | b'[', _] => {
                        ...
                        _ = traversal(state).parse_next(input)?;
                        continue;
                    }
                    // Conditional.
                    [b'?', _] => {
...
fn traversal_operator<'a>(input: &mut Input<'a>) -> PResult<'a, TraversalOperator> {
    dispatch! {any;
        b'.' => prefix_decorated(
            ws,
            dispatch! {peek(any);
...
fn unary_operator<'a>(input: &mut Input<'a>) -> PResult<'a, UnaryOperator> {
    dispatch! {any;
...
fn binary_operator<'a>(input: &mut Input<'a>) -> PResult<'a, BinaryOperator> {
    dispatch! {any;
...
fn object_key<'a>(input: &mut Input<'a>) -> PResult<'a, ObjectKey> {
    suffix_decorated(
        expr.map(|expr| {
...
fn object_value<'a>(input: &mut Input<'a>) -> PResult<'a, ObjectValue> {
...
fn object_value_assignment<'a>(input: &mut Input<'a>) -> PResult<'a, ObjectValueAssignment> {
    dispatch! {any;
...
fn for_intro<'a>(input: &mut Input<'a>) -> PResult<'a, ForIntro> {
    prefix_decorated(
        ws,
        delimited(
            b"for",
...
fn for_cond<'a>(input: &mut Input<'a>) -> PResult<'a, ForCond> {
    prefix_decorated(
        ws,
...
fn heredoc_start<'a>(input: &mut Input<'a>) -> PResult<'a, (bool, &'a str)> {
    terminated(
        (
...
fn func_args<'a>(input: &mut Input<'a>) -> PResult<'a, FuncArgs> {
    #[derive(Copy, Clone)]
    enum Trailer {
        Comma,
        Ellipsis,
