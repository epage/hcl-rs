...
pub(super) fn number<'a>(input: &mut Input<'a>) -> PResult<'a, Number> {
    alt((
...
fn integer<'a>(input: &mut Input<'a>) -> PResult<'a, u64> {
    digit1
        .try_map(|s: &[u8]| {
...
fn float<'a>(input: &mut Input<'a>) -> PResult<'a, f64> {
...
fn exponent<'a>(input: &mut Input<'a>) -> PResult<'a, &'a [u8]> {
    (
        one_of(b"eE"),
        opt(one_of(b"+-")),
